...
class PinterestAccountRedis:
    ...
    def remove_cookie(self, user_name):
        ...
        keys = self.redis.r_keys(redis_account_key + '*' + user_name)
        logging.info('find keys:', keys)

    def r_get(self, key: str):
        return self.redis.r_get(g_key(key))

    def get_random_cookie(self, user_type):
        # noinspection PyBroadException
        try:
            for i in range(5):
                key = self.redis.r_srandmember(
                    (redis_cookie_pre_key + ':' + str(user_type)) if user_type else redis_account_key)[0]
                data = self.r_get(key)
                if data:
                    return key, data
        except Exception:
...
rs = PinterestAccountRedis('dev').redis.r_keys(redis_account_key + ':*')
print(rs)
